...
module.exports = function (Table) {
            ...
            Table.findOne({ appId: appId, name : data.name }, function (err, table) {
                if (err) {
                    ...
                    if (data._id) {
                        ...
                        setAndSaveTable(appId, data, table, originalTable)
		              	.then(function (savedTable) {
                            createTable(appId, savedTable.name, savedTable.columns);
                            deferred.resolve(savedTable);
                        }, function (error) {
                            deferred.reject(error);
                        });
    ...
    function createTable(appId, tableName, columns) {
        
        var deferred = Q.defer();
        
        schema = JSON.stringify(schema);
        var post_data = "{ \"key\" : \"" + keys.cbDataServicesConnectKey + "\",\"columns\" : " + columns + "}";
        request.post({
            headers: {
                'content-type' : 'application/json',
                'content-length' : post_data.length
            ...
            if (error) {
                ...
                if (response.body === 'Success') {
                    ...
                    deferred.reject("Cannot create table.");
                }
            }
