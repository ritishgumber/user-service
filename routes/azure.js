...
module.exports = function () {
  //app.post('/webhooks/azure/subscriptions/:subscription_id/Events', subscription);
...
function onSubscriptionRegistered(req, res) {
  var email = req.body.Properties.Email;
  //var optin = req.body.Properties.OptIn;
...
function onSubscriptionUnregistered(req, res) {
  ...
  var newJson = { isActive: false,azure: {subscription_id: req.params['subscription_id'],optin: false} };

  global.userService.getAzureUserByEmail(query, newJson).then(function (data) {
...
function getNewToken(req, res, next) {
  ...
  var resourceId = "testid";
  var response = utils.loadTemplate('sso.xml', { token: token, resourceId: resourceId });
  res.setHeader('Content-Type', 'application/xml');
