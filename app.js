        ...
        try {
            ...
            if (global.config && global.config.redis && global.config.redis.length > 0) {
                ...
                for (var i = 0; i < global.config.redis.length; i++) {
                    hosts.push({host: global.config.redis[i].host, port: global.config.redis[i].port});
    ...
    function setUpMongoDB(passport) {
        ...
        try {
            ...
            global.app.use(expressWinston.errorLogger({
                transports: [
                    new global.winston.transports.Console({json: true, colorize: true}),
                    new global.winston.transports.Loggly({subdomain: 'cloudboost', inputToken: global.keys.logToken, json: true, tags: ["frontend-server"]})
                ]
            }));
    ...
    function sessionConfiguration() {
        try {
            ...
            global.app.use(session({
                key: 'session', resave: false, //does not forces session to be saved even when unmodified
